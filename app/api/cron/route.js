...
function buildExcerpt(generated, story) {
  // metaDesc: single line, already cleaned in parseResponse
  if (generated.metaDesc && generated.metaDesc.length > 30 && !generated.metaDesc.includes('##')) {
    const clean = cleanText(generated.metaDesc);
    if (clean.length > 30) return clean.slice(0, 200);
  }
  // hook: first paragraph of content
  if (generated.content?.hook) {
    const plain = cleanText(generated.content.hook);
    if (plain.length > 30) return plain.slice(0, 200);
  }
  // first section body
  if (generated.content?.sections?.[0]?.body) {
    const plain = cleanText(generated.content.sections[0].body);
    if (plain.length > 30) return plain.slice(0, 200);
  }
  // rawContent first sentence
  if (generated.rawContent) {
    const plain = cleanText(generated.rawContent);
